...
这套流程用于你在设备上手动操作 App，同时在电脑侧记录步骤，并生成这个仓库可执行的 Appium + pytest 测试草稿。手动录制转脚本当前面向 iOS；Bug 录制同时支持 iOS 和 Android。

## 1. 启动 iOS 手动录制

先确保 Appium server 已经启动，并且 iOS 设备能通过现有 `appium:ios:preflight`。

```bash
pnpm appium:ios:record -- --session-name message-search
```

录制器连上设备后，会先抓一份初始页面状态。之后每当你在手机上完成一个动作，就在终端输入一条命令：
...
Bug 录制适合先沉淀复现步骤、期望/实际结果和截图证据，再决定是否生成脚本。
...
.tmp/appium-android/recordings/<session-name>/
```

目录中会包含：

- `recording.json`: 结构化录制结果
- `bug-report.md`: 人工确认用报告
- `taiga-issue.md`: 可直接整理进 Taiga issue 的描述草稿

当前 Taiga MCP 不支持附件上传，所以截图和 XML 证据会以本地路径形式写入 Markdown 报告。
